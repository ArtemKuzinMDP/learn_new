...
# - Кольца защиты и современные ОС.	Кольца защиты разделяют доступы на 4 кольца, что позволяет на аппаратном уровне разграничить опасность влияния программ, повышая отказоустойчивость. 
...
# Философия (всё есть файлы)	нет никаких дисков. нет поддержки структурированных файлов ядром ОС. переменные окружения. клонирование текущего процесса со всем состоянием. Все файлы сходятся в одну точку - руут. система файлов в значении структуры каталогов операционной системы - древовидная. 
...
# Стандарт POSIX	Посикс - язык запросов программ к ядру, и ядра к оборудованию. 
...
3. Структура каталогов Линукс.	строгая иерархическая структура. все ветки ведутся от root, который является корневым, главным. в нем содержится каталоги 
