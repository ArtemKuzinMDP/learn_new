...
Посмотреть из чего состоит promt, можно вызвав переменную $PS1, например через echo. Управление переменными. Переменные, в том числе новые можно писать, после них знак равно и начальные данные, которые вы хотите им присвоить. Если нужно присвоить не текст или цифру, а переменную, то переменная выделяется кавычками. Экранимруемые последовательности: 
    ...
    Также при использовании экранированной последовательности можно перемещать курсор. Хранить все изменения можно в файле .bashrc, прописав там нужную команду и указав "export PS1". 
...
- rpm - работа с файлами в пакетах на диске, пакетный менеджер, поиск, удаление пакетов, сбор информации по файлам в пакете. пакетный менеджер, поиск, удаление пакетов, сбор информации по файлам в пакете
...
11) Управление данными через Redis
...
Ansible - система конфигурации и настройки для серверов Windows и Linux. Позволяет быстро при помощи написания скрипта поднять нужное на сервере программное обеспечение или разместить что-то. 

Читает текстовый формат Yaml, который имеет расширение .yml. 
Начинается этот файл всегда с "---" трех минусов на отдельной верхней строчке. Заканчивается тремя точка на последней строке. 
Создание списков начинается с - тире, которое ставится в начале каждого пункта списка. Если список многоуровневые, то на нижних уровнях ставится больше, но всегда одинаково пробелов перед - тире. Если не нижний уровень списка, но просто перечисление, то - тире не ставится, но число пробелов сохраняется. 
Можно прописывать аттрибуты и в строчку, для этого они обрамляются фигурными скобками, а внутри пишутся аттрибуты через двоеточие значения, значения через запятую друг от друга. 
Перечисления в строчке записываются в квадратных скобках, каждое значение в одиночных кавычках, через запятую друг от друга. 

Работает по формату Pull - когда на сервере уже есть установленный Agent, который позволяет брать настройки мастер. 
Либо по формату Push - без установленных программ, Master просто принудительно устанавливает свои конфигурации. 

Для размещения Control или Master server нужны Linux, Python 2.6 или 3.5. На серверах Control или Managed нужен логин и пароль от Linux или SSH-key и Python 2.6. Для Windows логин и парлоль, а также PowerShell 3 и специальный скрипт. Работа с Linux происходит через SSH и порт 22. Они должны быть всегда открыты. C Windows через WinRM и порт 5986, подключение Windows чуть более сложное и требует больше обязательных строчек при описании сервера. Подробная инструкция расположена на docs.ansible.com. 

Конфигурации формируются в специальных файлах, которые называются Inventory. Создаете текстовый файл и в нем при помощи групп данных формируете нужные значения. 

Группы, например [staging_servers] пишутся в квадратных скобках и обобщают группы серверов, с которыми можно делать разные действия. Основной файл - файл конфига, его лучше держать в самом проекте. Создаете текстовый файл с разрешением cfg. Первая строка всегда [defaults], под которой пишутся значения по умолчанию. 
Правила создания файла Inventory - можно просто прописытвать IP-адрес или имена доменные, можно под alias прятать ip-адрес. Для создания групп в квадратных скобках мы пишем название группы, а также при помощи синтакиса [Название_Группы:children] мы можем указать и другие группы, которые будут входить в эту группу. 
Группы объединяются для того, чтобы стандартизировать переменные, с которыми они работают и прописать эти переменные только 1 раз. Прописывается как правило в отдельной Группе [Название_Группы:vars]. Все конфигурации в Ansible регистрозависимые - маленькие и большие буквы создают разные сущности. 
Чтобы inventory не пестрил множеством переменных, их выделяют отдельно. Для этого в директории, где лежит inventory, создается директория group_vars, а в нее добавляются текстовые файлы .yml, каждый с названием одной из групп серверов. Там храните все переменные, единственное отличие такого файла - вместо "=" для придания значения переменной используется ":". 

Чтобы управлять Ansible используются как текстовые файлы конфигурации, так и прямые команды, которые вводятся в терминале. Эти команды называются Ad-hog комманды. Чаще это команды удаленным управлением серверами. Самая частая команда - shell для запуска программ на других серверах. Которую также можно запустить с аттрибутом трабблшутинга от -v до -vvvvv.

Команды  Ansible:
- ansible - запускает ансибл 	
    - ansible -i - указывает инвентори, из которого будет запускаться, 
    - ansible -i &&& -m - указывает модуль, который будет запускаться	
- ansible-inventory - выводит все сервера, прописанные в inventory, выводит по группам, с переменными, которые они используют	
    - ansible-inventory --list - выводит отдельно информацию о серверах. 
    - ansible-inventory --graph - выводит дерево зависимости групп и серверов	
- ansible &&& -m ping - Модуль проверяет коннект со всеми хостами в группе. Вместо &&& группа хостов. 
- ansible &&& -m setup - Модуль выводит настраиваимую информацию обо всех серверах в группе: Вместо &&& группа хостов. 
- ansible &&& -m shell -a "" - Модуль выводит указанную в кавычках команду на указанной группе серверов	
    - -v - вывод информации о конфигурационном файле, используемом при запуске, 
    - -vv - вывод также информации о версии ансибл 
    - -vvv - базовый трабблшутинг по команде с перечнем действий, какой юзер, какой ключ, 
    - -vvvv углубенный трабблшутинг. 
    - -vvvvv - тяжелый и подробный лог происходящего	Вместо &&& группа хостов. 
- ansible &&& -m command -a ""	Модуль выводит указанную в кавычках команду на указанной группе серверов, но не через shell, поэтому не видит переменные, а также значки сочетания команд. Вместо &&& группа хостов. 
- ansible &&& -m copy -a "src= dest=" - Модуль копирует указанный файл на группу серверов	src= - это полный путь до файла,  dest= - это директория, куда скопировать, mode= - это изменение прав, ставим число прав, -b - это предоставление прав sudo. место &&& группа хостов. 
- ansible &&& -m file -a "path= state=" - Модуль позволяет производить разные операции с файлами	path= - это путь до файла, state= - это предание значения операции с файлом. (Например absent - удалить). -b - запуск от имени суперюзера	Вместо &&& группа хостов. 
- ansible &&& -m get_url -a "url= dest=" - Модуль позволяет скачивать файлы с интернета на указанную группу серверов	url= - это ссылка на файл в интернет, обязательно с http. dest= - это расположение, куда нужно что-то скачать. -b - запуск от имени суперюзера. Вместо &&& группа хостов. 
- ansible &&& -m yum -a "name= state=" - Модуль позволяет устанавливать выбранные команды на группу серверов	name= - название файла, который нужно скачать, state= - что сделать (latest - обновить до последней версии, installed - установить, revomed - удалить). -b - запуск от имени суперюзера. Вместо &&& группа хостов. 
- ansible &&& -m uri -a "url= dest=" - Модуль позволяет читать данные с сайта на указанной группе серверов	url= - это ссылка на файл в интернет, обязательно с http. return_content=yes - позволяет на серверах прочитать данные страницы этого сайта. Вместо &&& группа хостов. 
- ansible &&& -m service -a "name= state= enabled=" - Модуль запускает указанный сервис на группе серверов	name= - это название сервиса. state= - это указание действия (started - запустить). enabled= - это аттрибут запуска при старте сервера (да или нет). -b - запуск от имени суперюзер. Вместо &&& группа хостов. 
- ansible-doc -l - Показывает все команды, которые есть в ansible
